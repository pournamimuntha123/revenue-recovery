...
python3 generate_data.py          # creates data.csv (120 synthetic transactions)
python3 main.py                   # runs the full pipeline and prints the report
```

Output:
- Console report with headline numbers
- `audit_log.csv` — full decision trail for every transaction
...
`simulate_outcomes.py` for the assumed rates.)*
...
- `recovery_engine.py` — classification + decision + stopping rules
- `simulate_outcomes.py` — simulates outcomes and computes metrics
- `main.py` — runs everything end-to-end
- `data.csv` — generated input data
- `audit_log.csv` — generated output (full decision trail)
